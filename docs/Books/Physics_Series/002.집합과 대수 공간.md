...
* **집합(Set)**은 잘 정의된 객체들의 모음이다(Well-defined collection of objects).
* 집합이 포함하는 객체는 **원소(elements)**라 부른다.
* $X$가 집합이고 $a$가 집합 $X$의 원소라면, $X$가 $a$를 포함한다, $a$가 $X$에 속한다라 한다.  $z \in X$로 표기한다.
* $X$가 집합이고 $a$가 집합 $X$의 원소가 아니라면, $z \not\in X$로 표기한다.
 
자연 언어로 정의되었긴 하지만, 술어 논리의 언어로 형식화 시킬 수도 있다. 
...
<th style="text-align:center;background-color:rgb(6 , 0, 0); color:white; border-top-left-radius: 10px;width:20%;">
Def</th>
