...
이 라이브러리는 C 언어 BLAS 표준(CBLAS라고 합니다.)에 해당하는 저수준의 기능들과 고수준의 GSL 벡터, 행렬들을 위한  기능들을 제공합니다. GSL 벡터와 행렬의 연산을 하고자 한다면, 여기서 기술하는 고차원 기능들을 사용할 수 있습니다. 이 단원에서 서술하는 함수들은 `gsl_blas.h`파일에 정의되어 있고 대부분의 사용자들의 요구를 충족시킬 수 있습니다.
...
|Level 2|행렬-벡터 연산. $y = \alpha A x + \beta y$|
|level 3|행렬-행렬 연산, $C = \alpha A B + C$|
...
예를 들어서, `SGEMM`은 "단 정밀도 실수 일반 행렬-행렬 곱"을 의미하고 `ZGEMM`은 "배 정밀도 복소수 일반 행렬-행렬 곱"을 의미합니다.

## Level 1 BLAS 인터페이스

>int **gsl_blas_sdsdot**(float *alpha*, const gsl_vector_float * *x*, const gsl_vector_float * *y*, float * *result*)

이 함수는 $\alpha + x^T y$ 값을 주어진 벡터 `x`, `y`에 대해 계산하고 결과를 `result`에 반환합니다.
...
이 함수들은 스칼라 곱 $x^T y$를 주어진 벡터 `x`, `y`에 대해 계산하고 결과를 `result`에 반환합니다.
...
이 함수들은  복소수 스칼라 곱 $x^T y$를 주어진 벡터 `x`, `y`에 대해 계산하고 결과를 `result`에 반환합니다.
...
이 함수들은 주어진 벡터 `x`의 원소중 가장 큰 원소의 인덱스를 반환합니다. 가장 큰 원소는 실수 벡터의 경우 원소의 절대값의 크기가 가장 큰 경우, 복소수 벡터의 경우 실, 허수 부분의 크기의 합 $|\Re(x_i)| + |\Im(x_i)|$이 가장 큰 경우로 결정됩니다. 만약, 가장 큰 원소가 여러개 있다면, 첫번째 원소가 반환됩니다.
...
이 함수들은 주어진 벡터 `x`, `y`의 원소들을 교환합니다.
...
이 함수들은 주어진 벡터 `x`의 원소들을 `y`로 복사합니다.
...
이 함수들은 $y=\alpha x + y$의 값을 주어진 벡터 `x`, `y`에 대해 계산합니다.
...
이 함수들은 벡터 `x`의 원소들의 크기를 주어진 값 `alpha`를 곱해 변경합니다.

>int **gsl_blas_srotg**(float *a[]*, float *b[]*, float *c[]*, float *s[]*)<br>
>int **gsl_blas_drotg**(double *a[]*, double *b[]*, double *c[]*, double *s[]*)

이 함수들은 벡터 $(a,b)$를 0으로 만드는 기븐스(Givens) 회전 $(c,s)$를 계산합니다.

$$\begin{pmatrix}c & s\\ -c & s\end{pmatrix}  \begin{pmatrix} a\\ b \end{pmatrix} = \begin{pmatrix}r'\\ 0\end{pmatrix}$$

변수 `a`, `b`들은 명령어에 의해 여러번 초기화됩니다.
...
이 함수들은 회전 $(x',y') = (cx+sy, -sx+cy)$을 주어진 벡터 `x`, `y`에 대해 적용합니다.
...
이 함수들은 행렬-벡터 사이의 곱, 덧셈인 $y = \alpha op(A) x + \beta y$을 계산합니다. $op(A) = A, A^T, A^H$ 이고 `TransA` = `CblasNoTrans`, `CblasTrans`, `CblasConjTrans` 가 가능합니다.
...
>int **gsl_blas_ctrmv**(CBLAS_UPLO_t  *Uplo*, CBLAS_TRANSPOSE_t  *TransA*, CBLAS_DIAG_t  *Diag*, const gsl_matrix_complex_float *  *A*, gsl_vector_complex_float *  *x*)<br>
...
이 함수들은 행렬-벡터 곱 $x = op(A)x$를 삼각 행렬 `A`에 대해 계산합니다. $op(A) = A, A^T, A^H$ 이고 `TransA` = `CblasNoTrans`, `CblasTrans`, `CblasConjTrans` 가 가능합니다.`Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬이 사용됩니다. 만약, `Dig`가 `CblasNonUnit`이라면 행렬의 대각 성분들이 사용됩니다. `CblasUnit`이라면 행렬 `A`의 대각 성분들은 [유니티] 참조되지 않습니다. 
...
이 함수들은 주어진 벡터 `x`에 대해, $inv(op(A))x$를 계산합니다. $op(A) = A, A^T, A^H$ 이고 `TransA` = `CblasNoTrans`, `CblasTrans`, `CblasConjTrans` 가 가능합니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬이 사용됩니다. 만약, `Dig`가 `CblasNonUnit`이라면 행렬의 대각 성분들이 사용됩니다. `CblasUnit`이라면 행렬 `A`의 대각 성분들은 [유니티] 참조되지 않습니다. 
...
이 함수들은 행렬-벡터 곱과 합 $y = \alpha Ax + \beta y$을 대칭 행렬 `A`에 대해 계산합니다. 행렬 `A`가 대칭 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다. 
...
이 함수들은 행렬-벡터 곱과 합 $y = \alpha Ax + \beta y$을 에르미트 행렬 `A`에 대해 계산합니다. 행렬 `A`가 대칭 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다. 대각 성분의 복소 성분들은 자동적으로 0으로 가정하고 참조되지 않습니다. 
...
이 함수들은 행렬 `A`에 대해, 랭크-1 갱신 $A = \alpha xy^T + A$ 를 계산합니다.
...
이 함수들은 행렬 `A`에 대해, 컬례 랭크-1 갱신 $A = \alpha xy^H + A$ 를 계산합니다.
...
이 함수들은 대칭 행렬 `A`에 대해, 대칭 랭크-1 갱신 $A = \alpha xx^T + A$ 를 계산합니다. 행렬 `A`가 대칭 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다.
...
이 함수들은 에르미트 행렬 `A`에 대해, 에르미트 랭크-1 갱신 $A = \alpha xx^H + A$ 를 계산합니다. 행렬 `A`가 에르미트 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다. 대각 성분의 복소 성분들은 자동적으로 0으로 가정하고 참조되지 않습니다. 
...
이 함수들은 대칭 행렬 `A`에 대해, 대칭 랭크-2 갱신 $A = \alpha xy^T + \alpha yx^T + A$ 를 계산합니다. 행렬 `A`가 대칭 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다.
...
이 함수들은 에르미트 행렬 `A`에 대해, 에르미트 랭크-2갱신 $A = \alpha xy^H + \alpha yx^H + A$ 를 계산합니다. 행렬 `A`가 에르미트 행렬이기 때문에, 상삼각 부분이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다. 대각 성분의 복소 성분들은 자동적으로 0으로 가정하고 참조되지 않습니다. 
...
이 함수들은 행렬-행렬 사이의 곱과 합 $C =\alpha op(A)op(B) + \beta C$를 계산합니다. $op(A) = A, A^T, A^H$ 이고 `TransA` = `CblasNoTrans`, `CblasTrans`, `CblasConjTrans` 가 가능합니다. `TransB`도 같은 인자를 사용가능합니다.
...
이 함수들은 행렬-행렬 사이의 곱과 합을 계산합니다. `Side`가 `CblasLeft`면 $C =\alpha AB + \beta C$를, `CblsRight`면 $C =\alpha BA + \beta C$를 계산합니다. 행렬 `A`는 대칭 행렬이어야 합니다. `Uplo`가 `CblasUpper`일 때, 행렬 `A`의 상삼각 행렬과 대각 성분들이 사용되고, `Uplo`가 `CblasLower`일 때, `A`의 하삼각 행렬과 대각 성분들이 사용됩니다. 대각 성분의 복소 성분들은 자동적으로 0으로 가정하고 참조되지 않습니다. 
...
이 함수들은 행렬-행렬 곱을 계산합니다. `Side`가 `CblasLeft`면, $B= \alpha op(A)B$를, `CblsRight`면 $B=\alpha Bop(A)$를 계산합니다. 행렬 `A`는 에르미트 행렬이어야 합니다. `Uplo`가 `CblasUpper`인 경우, 행렬 `A`의 상삼각 부분과 대각 부분이 사용되고, `CblasLower`인 경우 하삼각 부분과 대각 부분이 사용됩니다. 허수 성분의 대각 성분은 자동으로 0으로 취급됩니다.
...
이 함수들은 행렬-행렬 곱을 계산합니다. `Side`가 `CblasLeft`면 $B = \alpha op(A)B$를, `CblasRight`면 $B = \alpha B op(A)$를 계산합니다. 행렬 `A`는 삼각행렬이어햐 하고, `TransA`= `CblasNoTrans`, `CblasTrans`, `CblasConkTrans`인 경우 각각 $op(A) = A, A^T, A^H$를 의미합니다. `Uplo`가 `CblasUpper`인 경우 행렬 `A`의 상삼각 부분이 사용되고, `CblasLower`인 경우 `A`의 하삼각 부분이 사용됩니다. 만약, `Diag`가 `CblasNonUnit`이라면 행렬 `A`의 대각 성분이 사용되고, `CblasUnit`이라면 유니터리 취급하여 대각 성분은 참조되지 않습니다.
...
이 함수들은 역행렬의 행렬곱을 계산합니다. `Side`가 `CblasLeft`일 경우 $B=\alpha op(inv(A))B$를, `CblasRight`일 경우에는 $B=\alpha B op(inv(A))$를 계산합니다. 행렬 `A`는 삼각 행렬이어야 하고 `TransA`= `CblasNoTrans`, `CblasTrans`, `CblasConkTrans`인 경우 각각 $op(A) = A, A^T, A^H$를 의미합니다. `Uplo`가 `CblasUpper`면 행렬 `A`의 상삼각 부분이 사용되고, `CblasLower`인 경우 `A`의 하삼각 부분이 사용됩니다. 만약, `Diag`가 `CblasNonUnit`이라면 행렬 `A`의 대각 성분이 사용되고, `CblasUnit`이라면 유니터리 취급하여 대각 성분은 참조되지 않습니다.
...
이 함수들은 대칭 행렬 `C`의 랭크-k 갱신을 계산합니다. `Trans`가 `CblasNoTrans`이라면 $C = \alpha AA^T  + \beta C$을, `CblasTrans`리면 $C = \alpha A^T A + \beta C$를 계산합니다. 행렬 `C`가 대칭 행렬이기 때문에 상삼각이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`면 행렬 `C`의 상삼각 부분과 대각 부분이 사용되고, `CblasLower`인 경우 `C`의 하삼각 부분과 대각 부분이 사용됩니다. 
...
이 함수들은 에르미트 행렬 `C`의 랭크-k 갱신을 계산합니다. `Trans`가 `CblasNoTrans`이라면 $C = \alpha AA^H  + \beta C$을, `CblasTrans`리면 $C = \alpha A^H A + \beta C$를 계산합니다. 행렬 `C`가 에르미트 행렬이기 때문에 상삼각이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`면 행렬 `C`의 상삼각 부분과 대각 부분이 사용되고, `CblasLower`인 경우 `C`의 하삼각 부분과 대각 부분이 사용됩니다. 
...
이 함수들은 대칭 행렬 `C`의 랭크-2k 갱신을 계산합니다. `Trans`가 `CblasNoTrans`이라면 $C = \alpha AB^T + \alpha B A^T  + \beta C$을, `CblasTrans`리면 $C = \alpha A^T B+\alpha B^T A + \beta C$를 계산합니다. 행렬 `C`가 대칭 행렬이기 때문에 상삼각이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`면 행렬 `C`의 상삼각 부분과 대각 부분이 사용되고, `CblasLower`인 경우 `C`의 하삼각 부분과 대각 부분이 사용됩니다. 
...
이 함수들은 에르미트 행렬 `C`의 랭크-2k 갱신을 계산합니다. `Trans`가 `CblasNoTrans`이라면 $C = \alpha AB^T + \alpha B A^T  + \beta C$을, `CblasTrans`리면 $C = \alpha A^T B+\alpha B^T A + \beta C$를 계산합니다. 행렬 `C`가 에르미트 행렬이기 때문에 상삼각이나 하삼각 부분만 저장해도 됩니다. `Uplo`가 `CblasUpper`면 행렬 `C`의 상삼각 부분과 대각 부분이 사용되고, `CblasLower`인 경우 `C`의 하삼각 부분과 대각 부분이 사용됩니다. 
