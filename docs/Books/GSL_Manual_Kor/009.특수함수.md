...
이 함수들은 $0$이나 음의 정수가 아닌 $x$에 대해, 로그 감마 함수 $\log(\Gamma(x))$의 값을 계산합니다. $x<0$에 대해, $\log(\Gamm(x))$값이 반환되며, 이 값은 $\log(|\Gamma(x)|)$의 값과 같습니다. 이 함수는 실수 Lanczos 방법을 사용합니다.
...
> int **gsl_sf_mathieu_ce_array**(int nmin, int nmax, double q, double x, gsl_sf_mathieu_workspace *work, double result_array[])<br>
