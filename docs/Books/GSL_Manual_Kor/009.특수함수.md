...
$$D(\phi, k) = \frac{1}{3}(\sinn \phi)^3 RD(1-\sin^2(\phi), 1-k^2\sin^2(\phi),1)$$
...
이훔스는 지수 함수 $\exp(x)$의 값을 계산하는 데, 반환값의 자료형으로`gsl_sf_result_e10`을 사용해 확장된 크기의 반환값을 계산합니다. 이 함수는 `double` 자료형의 범주를 초과한 $\exp(x)$값을 구할 때, 사용할 수 있습니다.
